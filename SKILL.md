...
- **`${CLAUDE_SKILL_DIR}/references/sensor-creation-guide.md`** — Complete sensor creation workflow: schema discovery, metascan, POST body construction, known bugs and workarounds, duplicate/resume detection. **MANDATORY: read this before creating any sensor.** It contains critical gotchas (e.g. metascan fields must be arrays, some sensors don't accept `name`) that cannot be guessed from the endpoint docs alone.
...
36. **Kind ID version suffixes must be stripped for comparison** — Kind IDs may have a numeric suffix (`paessler.snmp.cpu_usage_sensor.65`). This is a schema version identifier tied to the target device, not a different sensor type. Always strip trailing `.\d+` (`re.sub(r'\.\d+$', '', kind_id)`) when checking for duplicates or matching kinds. The schemas list endpoint returns unversioned IDs; the POST `?kindid=` parameter accepts either form.
37. **Schema list silently truncates at 100** — Always pass `limit=1000` to `GET /experimental/schemas/{deviceId}` or you'll see only ~100 of ~329 available sensor kinds.
38. **`include=all_sections` is mandatory for POST schema** — `GET /experimental/schemas/{id}/post?kind={kind}` without `include=all_sections` omits required fields from sensor-specific sections. Always add this parameter.
39. **Metascan body requires `{"params": {...}}` wrapper for credential overrides** — When a metascan needs per-sensor credentials (e.g. NetApp, custom SSH params), wrap them in a `"params"` key: `{"params": {"username": "x", "password": "y"}}`. Without the wrapper, the credentials are silently ignored.
40. **`_inherited` must be set explicitly on some POST bodies** — Sections like `scriptplaceholdergroup` and `snmpversiongroup` have `_inherited` with `default: true` in the schema, but the server rejects the request with "NULL value not allowed" if the field is omitted. Explicitly set `{"_inherited": true}` in these sections on POST. Affected kinds: `sshscript`, `sshscriptxml`, `snmpciscoasavpnconnections`, `snmpciscoasavpnusers`.
41. **`snmpcustomadvanced` disabled channels are still validated** — Setting `usechannel{N}: "0"` does NOT suppress validation of `channel{N}name` (required). Workaround: provide placeholder names and explicit `unit: "Count"` for all 10 channels even when disabled.
42. **POST schema field names ≠ UI labels** — e.g. the "Channel Name" field is `oidgroup.channel`, not `oidgroup.channelname`. The API rejects unknown field names outright. Always inspect the POST schema for exact field keys.
43. **Sections to skip on POST** — These sections appear in GET/PATCH schemas but must NOT be sent in POST: `scheduledependency`, `intervalgroup`, `channelgroup`, `status`, `id`, `kind`, `kind_name`, `parent`, `type`, plus all read-only metadata fields. See `sensor-creation-guide.md` for the full list.
44. **Resume by kind ID, not name** — Metascan sensors get names from their discovery data ("Disk Free: /" instead of "SNMP Disk Free"), so name-based duplicate checks fail. Check existing sensors via `GET /experimental/sensors?filter=parentid="{id}"` and compare by kind ID (with version suffix stripped).
